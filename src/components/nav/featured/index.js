    ...
    render={data => (
        ...
        {data.allContentfulBlog.edges.map(edge => (
            <li className="popular-article">
                <div className="thumbnail" style={{backgroundImage: `url(${edge.node.featuredImage.fluid.src})`}}></div>
                <div className="article-datail-container">
                    <Link to={`/blog/${edge.node.slug}/`} className="article-title-link">
                        <h4 className="article-title">{edge.node.title}</h4>
                    </Link>                           
                    <div className="topics">
                    {edge.node.topics.map(topic => (
                        <Link to={`/${topic.slug}/`} className="topic">{topic.title}</Link>
                    ))}
                    </div>
                    <div className="dates">
